  ...
  //map 
   CameraPosition _initialCameraPosition = const CameraPosition(
    ...
    customIcon = await BitmapDescriptor.fromAssetImage(
      ImageConfiguration(size: Size(15, 15)), // You can adjust the size if needed
...
Future<void> fetchRoute() async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      print("Failed to connect to the Google Maps API with status code: ${response.statusCode}");
        ...
        SizedBox(height: 20,),
         ...
         Container(
              alignment: Alignment.topLeft,
                ...
                SizedBox(height: 10,),
        //transit
        if(_controllerFrom.text.isNotEmpty && _controllerTo.text.isNotEmpty )
        Expanded(
  ...
  for (var step in leg['steps']) {
    ...
    if (step['travel_mode'] == 'TRANSIT') {
      ...
      if (transitDetails != null && transitDetails['line'] != null) {
        final vehicleType = transitDetails['line']['vehicle']['type'] ?? 'Unknown Vehicle';
    ...
    if (step['travel_mode'] == 'WALKING') {
      final duration = step['duration'] != null ? step['duration']['text'] ?? 'Unknown Duration' : 'Unknown Duration';
      transportNames += 'Walk - ';
    }
  }
  
  // Set the total duration for the leg
  totalDuration = leg['duration'] != null ? leg['duration']['text'] ?? 'Unknown Time' : 'Unknown Time';
}

// Remove the trailing " - " from transportNames
if (transportNames.endsWith(' - ')) {
  transportNames = transportNames.substring(0, transportNames.length - 3);
}


              // Customize this part based on your data structure
              return suggestRoute(
                transportNames, // Dynamic transport names
                '₱50.00', // Replace with actual fare if available
                totalDuration, // Total time
                SvgPicture.asset('assets/icons/bus2.svg'), // Replace with actual icon based on mode
              );
            },
          ),
        ),
  ...
  Widget suggestRoute(String transpoNames, String fare, String time, SvgPicture pic) {
  return Container(
